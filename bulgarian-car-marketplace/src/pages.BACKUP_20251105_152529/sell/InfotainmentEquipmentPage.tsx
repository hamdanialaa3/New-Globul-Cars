...
const InfotainmentEquipmentContainer = styled.div`
  min-height: 100vh;
...
const InfotainmentEquipmentPage: React.FC = () => {
  ...
  const handleBack = () => {
    ...
    navigate(`/sell/inserat/${vehicleType || 'pkw'}/ausstattung/komfort?${params.toString()}`);
  };
  ...
  const handleContinue = () => {
    ...
    if (selectedEquipment.length > 0) {
      params.set('infotainment', selectedEquipment.join(','));
    }
  ...
  return (
    <InfotainmentEquipmentContainer>
      <ContentWrapper>
        <HeaderCard>
          <Title>Инфотейнмънт оборудване</Title>
          <Subtitle>
            Изберете всички системи за инфотейнмънт, които има вашето превозно средство
          </Subtitle>
        </HeaderCard>

        <EquipmentCard>
          <EquipmentGrid>
            {infotainmentEquipment.map((equipment) => (
        ...
        <InfoCard>
          <InfoTitle>🎵 За инфотейнмънт оборудването</InfoTitle>
          <InfoText>
            Инфотейнмънт системите правят пътуването по-забавно и удобно. 
...
export default InfotainmentEquipmentPage;
