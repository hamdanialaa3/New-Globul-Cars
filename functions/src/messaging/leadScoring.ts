...
import { CalculateLeadScoreRequest, UpdateLeadStatusRequest, Lead } from './types';
...
function calculateScore(conversationData: any, messages: any[]): { score: number; breakdown: any; priority: 'hot' | 'warm' | 'cold' } {
  ...
  if (messageCount >= 10) engagement = 30;
  else if (messageCount >= 7) engagement = 25;
  else if (messageCount >= 5) engagement = 20;
  else if (messageCount >= 3) engagement = 15;
  else if (messageCount >= 1) engagement = 10;

  // Count questions (more questions = more engaged)
  const questionCount = messages.filter((m) => m.content?.includes('?')).length;
  if (questionCount >= 3) engagement = Math.min(30, engagement + 5);
  ...
  if (buyerMessages.length > 1) {
    ...
    for (let i = 1; i < buyerMessages.length; i++) {
      const prevTime = buyerMessages[i - 1].timestamp?.toMillis() || 0;
      const currentTime = buyerMessages[i].timestamp?.toMillis() || 0;
    ...
    if (avgResponseHours < 1) responseTime = 20;
    else if (avgResponseHours < 6) responseTime = 15;
    else if (avgResponseHours < 24) responseTime = 10;
    else if (avgResponseHours < 48) responseTime = 5;
  ...
  seriousKeywords.forEach((keyword) => {
    if (allContent.includes(keyword)) seriousKeywordCount++;
  });

  if (seriousKeywordCount >= 5) seriousness = 25;
  else if (seriousKeywordCount >= 3) seriousness = 20;
  else if (seriousKeywordCount >= 2) seriousness = 15;
  else if (seriousKeywordCount >= 1) seriousness = 10;
  ...
  specificQuestions.forEach((keyword) => {
    if (allContent.includes(keyword)) specificCount++;
  });
  if (specificCount >= 3) seriousness = Math.min(25, seriousness + 5);
  ...
  budgetKeywords.forEach((keyword) => {
    if (allContent.includes(keyword)) budgetKeywordCount++;
  });

  if (budgetKeywordCount >= 4) budget = 25;
  else if (budgetKeywordCount >= 3) budget = 20;
  else if (budgetKeywordCount >= 2) budget = 15;
  else if (budgetKeywordCount >= 1) budget = 10;
  ...
  else if (totalScore >= 40) priority = 'warm';
  else priority = 'cold';
 ...
 */
export const calculateLeadScore = onCall<CalculateLeadScoreRequest>(async (request) => {
  ...
  try {
    ...
    if (conversationData?.sellerId !== auth.uid) {
      throw new HttpsError('permission-denied', 'You can only score your own leads');
    }
    ...
    if (leadsSnapshot.empty) {
      ...
      const lead: Lead = {
        ...
        priority,
        notes: [],
        lastContactAt: conversationData?.lastMessageAt || Timestamp.now(),
 ...
 * Update lead status
 */
export const updateLeadStatus = onCall<UpdateLeadStatusRequest>(async (request) => {
  const { auth, data } = request;
  ...
  if (!leadId || !status) {
    throw new HttpsError('invalid-argument', 'Lead ID and status are required');
  }
  ...
  try {
    ...
    const conversationDoc = await db.collection('conversations').doc(leadData?.conversationId).get();
    if (conversationDoc.data()?.sellerId !== auth.uid) {
    ...
    if (notes) {
      updates.notes = [...(leadData?.notes || []), notes];
    }
 ...
 */
export const onConversationUpdate = onDocumentWritten('conversations/{conversationId}', async (event) => {
  const conversationData = event.data?.after.data();
