 ...
 */
export const aiQuotaCheck = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
...
export const geminiChat = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if ((quota?.usedChatMessages || 0) >= (quota?.dailyChatMessages || 10)) {
 ...
 */
export const geminiPriceSuggestion = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User not authenticated');
  }
 ...
 */
export const geminiProfileAnalysis = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User not authenticated');
  }
 ...
 */
export const sentimentAnalysis = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User not authenticated');
  }
 ...
 */
export const detectLanguageAndTranslate = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User not authenticated');
  }
 ...
 */
export const processVoiceMessage = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User not authenticated');
  }
...
// Export webhook
export const whatsapp = whatsappWebhook;
