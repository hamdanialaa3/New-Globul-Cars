...
import { TrustScoreFactors, TrustScoreResult } from './types';
...
export async function calculateTrustScore(userId: string): Promise<TrustScoreResult> {
  ...
  try {
    ...
    if (userData.lastActivityAt) {
      const lastActivity = (userData.lastActivityAt as Timestamp).toDate();
      const now = new Date();
  ...
  requiredFields.forEach((field) => {
    if (listing[field]) {
      if (Array.isArray(listing[field])) {
        if (listing[field].length > 0) filledCount++;
      } else {
        filledCount++;
      }
    }
 ...
 */
function getTrustLevel(score: number): 'beginner' | 'intermediate' | 'advanced' | 'expert' | 'elite' {
