...
interface BulgarianInsuranceProvider {
  id: string;
  name: string;
 ...
 */
export const getInsuranceQuotes = onCall(
  {
    region: 'europe-west1',
    cors: true
  },
  async (request) => {
    try {
      ...
      if (!vehicleId || !customerId) {
        throw new HttpsError('invalid-argument', 'Vehicle ID and Customer ID are required');
      }

      logger.info('Getting insurance quotes', { vehicleId, customerId, coverageType });
      ...
      logger.error('Error getting insurance quotes:', error);
      throw new HttpsError('internal', 'Failed to get insurance quotes');
    }
  }
);
 ...
 */
export const purchaseInsurancePolicy = onCall(
  {
    region: 'europe-west1',
    cors: true
  },
  async (request) => {
    try {
      ...
      if (!quoteId) {
        throw new HttpsError('invalid-argument', 'Quote ID is required');
      }

      logger.info('Purchasing insurance policy', { quoteId, paymentMethod, paymentFrequency });
      ...
      logger.error('Error purchasing insurance policy:', error);
      throw new HttpsError('internal', 'Failed to purchase insurance policy');
    }
  }
);
 ...
 */
export const getCustomerPolicies = onCall(
  {
    region: 'europe-west1',
    cors: true
  },
  async (request) => {
    try {
      ...
      logger.info('Getting customer policies', { customerId });
      ...
      logger.error('Error getting customer policies:', error);
      throw new HttpsError('internal', 'Failed to get customer policies');
    }
  }
);
 ...
 */
export const fileInsuranceClaim = onCall(
  {
    region: 'europe-west1',
    cors: true
  },
  async (request) => {
    try {
      ...
      if (!policyId || !claim) {
        throw new HttpsError('invalid-argument', 'Policy ID and claim data are required');
      }

      logger.info('Filing insurance claim', { policyId, claimType: claim.type });
      ...
      logger.error('Error filing insurance claim:', error);
      throw new HttpsError('internal', 'Failed to file insurance claim');
    }
  }
);
 ...
 * Get claim details
 */
export const getClaimDetails = onCall(
  {
    region: 'europe-west1',
    cors: true
  },
  async (request) => {
    try {
      ...
      if (!claimId) {
        throw new HttpsError('invalid-argument', 'Claim ID is required');
      }

      logger.info('Getting claim details', { claimId });
      ...
      throw new HttpsError('internal', 'Failed to get claim details');
    }
  }
);
 ...
 */
export const getInsuranceProviders = onCall(
  {
    region: 'europe-west1',
    cors: true
  },
  async (request) => {
    try {
      logger.info('Getting insurance providers');

      const providers = getBulgarianInsuranceProviders();

      return providers;
    } catch (error) {
      logger.error('Error getting insurance providers:', error);
      throw new HttpsError('internal', 'Failed to get insurance providers');
    }
  }
);
 ...
 */
export const getInsuranceMarketStats = onCall(
  {
    region: 'europe-west1',
    cors: true
  },
  async (request) => {
    try {
      logger.info('Getting insurance market statistics');
      ...
      logger.error('Error getting insurance market stats:', error);
      throw new HttpsError('internal', 'Failed to get insurance market statistics');
    }
  }
);
...
async function getBulgarianInsuranceQuotes(
  vehicleId: string,
  customerId: string,
  coverageType: string
...
async function getVehicleData(vehicleId: string): Promise<any> {
...
function calculateBasePremium(vehicleData: any, customerData: any, coverageType: string): number {
  ...
  if (customerData.age < 25) basePremium *= 1.5;
  if (customerData.drivingExperience < 5) basePremium *= 1.3;
  if (customerData.location === 'София') basePremium *= 1.2;
  if (customerData.hasAccidents) basePremium *= 1.4;
  if (coverageType === 'premium') basePremium *= 1.8;
  else if (coverageType === 'comprehensive') basePremium *= 1.3;

  return Math.round(basePremium);
}

function getCoverageDetails(coverageType: string, provider: string): InsuranceCoverage {
  const baseCoverage: InsuranceCoverage = {
    liability: {
...
function getDeductibles(coverageType: string): { [key: string]: number } {
  const deductibles = {
...
async function processInsurancePurchase(
  quoteId: string,
  ...
  logger.info('Insurance policy purchased', { policyId, quoteId });

  return policyId;
}

async function getCustomerInsurancePolicies(customerId: string): Promise<InsurancePolicy[]> {
...
async function submitInsuranceClaim(policyId: string, claimData: any): Promise<string> {
  ...
  logger.info('Insurance claim submitted', { claimId, policyId });

  return claimId;
}

async function getInsuranceClaimDetails(claimId: string): Promise<InsuranceClaim | null> {
...
function getBulgarianInsuranceProviders(): BulgarianInsuranceProvider[] {
  return [
    {
      id: 'bulstrad',
      name: 'Bulstrad',
