 ...
 */
export const merchantFeed = functions
 ...
 */
function generateMerchantFeedXML(cars: CarListing[], baseUrl: string): string {
 ...
 */
function generateCarItem(car: CarListing, baseUrl: string): string {
  // Generate numeric URL
  const carUrl = `${baseUrl}/car/${car.sellerNumericId}/${car.carNumericId}`;
  
  // Primary image URL
  const imageUrl = car.images[0]?.startsWith('http') 
    ? car.images[0] 
 ...
 */
export const updateMerchantFeedCache = functions
  .region('europe-west1')
...
export default { merchantFeed, updateMerchantFeedCache };
