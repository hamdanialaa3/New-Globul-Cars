...
export const createB2BSubscription = onCall({ cors: true, region: 'europe-west1' }, async (request) => {
  if (!request.auth) {
...
export const getB2BSubscription = onCall({ cors: true, region: 'europe-west1' }, async (request) => {
  if (!request.auth) {
    throw new Error('Authentication required');
  }
...
// Cancel subscription
export const cancelB2BSubscription = onCall({ cors: true, region: 'europe-west1' }, async (request) => {
  if (!request.auth) {
    throw new Error('Authentication required');
  }
...
export const upgradeB2BSubscription = onCall({ cors: true, region: 'europe-west1' }, async (request) => {
  if (!request.auth) {
    throw new Error('Authentication required');
  }
