...
export const exportB2BLeads = functions
  .region('europe-west1')
  .https.onCall(
    async (data: ExportLeadsRequest, context): Promise<ExportLeadsResponse> => {
      ...
      logger.info('B2B Lead export requested', {
        userId,
      ...
      try {
        ...
        logger.info('Found user cars for lead export', {
          userId,
          carCount: userCarIds.length,
        });
        ...
        for (let i = 0; i < userCarIds.length; i += batchSize) {
          ...
          for (const msgDoc of messagesSnapshot.docs) {
            ...
            leads.push({
              ...
              inquirerName:
                inquirerData.displayName || inquirerData.name || 'Anonymous',
          ...
          for (let i = 0; i < userCarIds.length; i += batchSize) {
            ...
            for (const inquiryDoc of inquiriesSnapshot.docs) {
              ...
              leads.push({
                ...
                createdAt:
                  inquiryData.createdAt?.toDate?.()?.toISOString() ||
                  new Date().toISOString(),
        ...
        logger.error('B2B Lead export failed', {
          userId,
...
export const getB2BAnalytics = functions
  ...
  .https.onCall(async (data: { dateRange: string; tier: string }, context) => {
    ...
    const userId = context.auth.uid;
    logger.info('B2B Analytics requested', {
      userId,
      dateRange: data.dateRange,
    });
    ...
    try {
      ...
      // Monthly aggregation tracking
      const monthlyData: Record<string, { count: number; totalPrice: number }> =
        {};

      // Price trend: compare last 30 days avg price vs previous 30 days
      const now = Date.now();
        ...
        snapshot.docs.forEach(doc => {
          ...
          if (data.status === 'active') {
            ...
            if (createdMs >= thirtyDaysAgo) {
              recentPrice += data.price || 0;
              recentCount++;
            } else if (createdMs >= sixtyDaysAgo) {
              prevPrice += data.price || 0;
              prevCount++;
            }

            // Monthly aggregation (YYYY-MM key)
            if (createdMs > 0) {
              ...
              monthlyData[monthKey].totalPrice += data.price || 0;
            }
      ...
      return {
        ...
        totalUsers,
        averagePrice:
          activeListings > 0 ? Math.round(totalPrice / activeListings) : 0,
        priceTrend: (() => {
          const recentAvg = recentCount > 0 ? recentPrice / recentCount : 0;
          const prevAvg = prevCount > 0 ? prevPrice / prevCount : 0;
          if (prevAvg === 0) return 0;
          return Math.round(((recentAvg - prevAvg) / prevAvg) * 1000) / 10; // % with 1 decimal
        })(),
        popularMakes,
        locationStats,
      ...
      logger.error('B2B Analytics failed', {
        userId,
        error: (error as Error).message,
      });
      throw new functions.https.HttpsError(
...
export const exportB2BAnalytics = functions
  .region('europe-west1')
  .https.onCall(
    async (data: { dateRange: string; format: string }, context) => {
      if (!context.auth) {
        throw new functions.https.HttpsError(
          'unauthenticated',
          'Must be authenticated'
      ...
      const rows = [
        ...
        ['', ''],
        ['Top Makes', ''],
        ...analytics.popularMakes.map(
          (m: { make: string; count: number; avgPrice: number }) => [
            m.make,
            `${m.count} listings, €${m.avgPrice} avg`,
          ]
        ),
      ];
