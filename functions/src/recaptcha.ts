...
if (!GCP_PROJECT_ID || !RECAPTCHA_SITE_KEY) {
    logger.error("GCP Project ID or reCAPTCHA Site Key is not configured in Firebase Functions");
}
...
export const verifyRecaptchaToken = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (!response.tokenProperties?.valid) {
      logger.error(`reCAPTCHA token is invalid: ${response.tokenProperties?.invalidReason}`);
      throw new functions.https.HttpsError('unauthenticated', 'reCAPTCHA validation failed.');
    }

    if (response.tokenProperties.action !== action) {
        logger.error(`reCAPTCHA action mismatch. Expected: ${action}, Got: ${response.tokenProperties.action}`);
    ...
    logger.info(`reCAPTCHA assessment score: ${score}`);
    ...
    logger.error('Error during reCAPTCHA verification', error instanceof Error ? error : new Error(String(error)));
    throw new functions.https.HttpsError(
      'internal',
