...
exports.getCommissionRate = exports.getAllCommissionPeriods = exports.getCommissionPeriod = exports.getCommissionPeriods = exports.onSaleCompleted = void 0;
 ...
 */
function calculateCommissionAmount(salePrice, profileType) {
    const rate = COMMISSION_RATES[profileType] || 0;
    return salePrice * rate;
}
/**
 * Auto-calculate commission when a sale is completed
 */
exports.onSaleCompleted = (0, firestore_1.onDocumentCreated)('transactions/{transactionId}', async (event) => {
    var _a, _b, _c, _d;
    ...
    try {
        ...
        const profileType = sellerData === null || sellerData === void 0 ? void 0 : sellerData.profileType;
        // Only dealers and companies pay commission
        if (!['dealer', 'company'].includes(profileType)) {
            return;
        }
        const commissionRate = COMMISSION_RATES[profileType];
        ...
        const listingTitle = listingDoc.exists
            ? `${(_b = listingDoc.data()) === null || _b === void 0 ? void 0 : _b.make} ${(_c = listingDoc.data()) === null || _c === void 0 ? void 0 : _c.model} ${(_d = listingDoc.data()) === null || _d === void 0 ? void 0 : _d.year}`
            : 'Unknown';
        ...
        if (!periodDoc.exists) {
            ...
            const existingPeriod = periodDoc.data();
 ...
 */
exports.getCommissionPeriods = (0, https_1.onCall)(async (request) => {
    const { auth } = request;
    if (!auth) {
 ...
 */
exports.getCommissionPeriod = (0, https_1.onCall)(async (request) => {
    ...
    try {
        ...
        if ((periodData === null || periodData === void 0 ? void 0 : periodData.userId) !== auth.uid) {
            ...
            if (!adminDoc.exists) {
                throw new https_1.HttpsError('permission-denied', 'You can only view your own commission periods');
            }
        }
        return {
            success: true,
            period: Object.assign({ id: periodDoc.id }, periodData),
        };
    }
    catch (error) {
 ...
 */
exports.getAllCommissionPeriods = (0, https_1.onCall)(async (request) => {
    const { auth } = request;
    if (!auth) {
        throw new https_1.HttpsError('unauthenticated', 'User must be authenticated');
    }
    ...
    const adminDoc = await db.collection('admins').doc(auth.uid).get();
    if (!adminDoc.exists) {
        throw new https_1.HttpsError('permission-denied', 'Only admins can view all commission periods');
    }
    try {
        const periodsSnapshot = await db
            .collection('commissionPeriods')
            ...
            .orderBy('period', 'desc')
            .get();
        const periods = periodsSnapshot.docs.map((doc) => (Object.assign({ id: doc.id }, doc.data())));
        // Calculate totals
        const totals = {
 ...
 */
exports.getCommissionRate = (0, https_1.onCall)(async (request) => {
    const { auth } = request;
    if (!auth) {
        throw new https_1.HttpsError('unauthenticated', 'User must be authenticated');
    }
    try {
        ...
        const profileType = userData === null || userData === void 0 ? void 0 : userData.profileType;
        const rate = COMMISSION_RATES[profileType] || 0;
        return {
            success: true,
            rate,
...
//# sourceMappingURL=calculateCommission.js.map