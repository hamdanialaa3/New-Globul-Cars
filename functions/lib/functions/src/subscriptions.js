...
exports.upgradeB2BSubscription = exports.cancelB2BSubscription = exports.getB2BSubscription = exports.createB2BSubscription = void 0;
...
exports.createB2BSubscription = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
    ...
    try {
        ...
        firebase_functions_1.logger.info('B2B subscription created', {
            dealerId: request.auth.uid,
            tier,
        ...
        firebase_functions_1.logger.error('Error creating B2B subscription:', error);
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        throw new Error(`Failed to create subscription: ${errorMessage}`);
    }
});
// Get subscription details
exports.getB2BSubscription = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
        throw new Error('Authentication required');
    }
    try {
        ...
        firebase_functions_1.logger.error('Error getting B2B subscription:', error);
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        throw new Error(`Failed to get subscription: ${errorMessage}`);
    }
});
// Cancel subscription
exports.cancelB2BSubscription = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
        throw new Error('Authentication required');
    }
    try {
        ...
        firebase_functions_1.logger.info('B2B subscription cancelled', {
            dealerId: request.auth.uid,
            subscriptionId
        });
        return {
        ...
        firebase_functions_1.logger.error('Error cancelling B2B subscription:', error);
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        throw new Error(`Failed to cancel subscription: ${errorMessage}`);
    }
});
// Upgrade subscription
exports.upgradeB2BSubscription = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
        throw new Error('Authentication required');
    }
    try {
        ...
        const subscriptionRef = subscriptionDoc.docs[0].ref;
        const currentSubscription = subscriptionDoc.docs[0].data();
        ...
        firebase_functions_1.logger.info('B2B subscription upgraded', {
            dealerId: request.auth.uid,
        ...
        firebase_functions_1.logger.error('Error upgrading B2B subscription:', error);
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        throw new Error(`Failed to upgrade subscription: ${errorMessage}`);
    }
});
// Track API usage (called by other functions)
async function trackAPIUsage(dealerId, endpoint) {
    try {
        const subscriptionDoc = await db.collection('b2bSubscriptions')
        ...
        firebase_functions_1.logger.error('Error tracking API usage:', error);
