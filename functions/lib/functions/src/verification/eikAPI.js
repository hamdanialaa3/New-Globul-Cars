...
exports.verifyEIKViaAPI = verifyEIKViaAPI;
exports.queryPublicRegistry = queryPublicRegistry;
exports.verifyEIKMultiSource = verifyEIKMultiSource;
exports.verifyEIKWithCache = verifyEIKWithCache;
/**
 ...
 */
async function verifyEIKViaAPI(eik) {
    try {
 ...
 */
function getMockEIKVerification(eik) {
 ...
 */
function validateEIKChecksum(eik) {
 ...
 */
async function queryPublicRegistry(eik) {
    try {
        // Public registry URL
 ...
 */
async function verifyEIKMultiSource(eik) {
    // Try API first
...
async function verifyEIKWithCache(eik) {
    const cleanEIK = eik.replace(/\s/g, '');
    // Check cache
    const cached = verificationCache.get(cleanEIK);
    if (cached && Date.now() - cached.timestamp < CACHE_DURATION) {
        console.log(`✅ Using cached verification for EIK: ${cleanEIK}`);
        return Object.assign(Object.assign({}, cached.result), { source: 'mock' }); // Indicate it's cached
    }
    // Verify
...
//# sourceMappingURL=eikAPI.js.map