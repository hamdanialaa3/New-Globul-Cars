...
exports.b2bMarketInsightsAPI = exports.b2bValuationAPI = void 0;
...
async function getB2BSubscriptionTier(userId) {
    try {
...
function checkAPILimits(tier, usage, endpoint) {
    var _a;
    const limits = {
    ...
    const limit = ((_a = limits[tier]) === null || _a === void 0 ? void 0 : _a.requests_per_month) || 0;
 ...
 */
exports.b2bValuationAPI = (0, https_1.onCall)({
    cors: true,
    region: 'europe-west1'
}, async (request) => {
    ...
    const { vin, make, model, year, mileage, condition, fuelType, transmission, engineSize, power, location, features, purpose // 'insurance_claim', 'loan_valuation', 'dealer_inventory', 'market_analysis'
     } = request.data;
    try {
        // Track API usage
        await (0, subscriptions_1.trackAPIUsage)(userId, 'valuation');
    ...
    `;
        ...
        if (engineSize) {
            if (engineSize > 3.0)
                adjustedPrice *= 1.1; // Large engines
            else if (engineSize < 1.2)
                adjustedPrice *= 0.95; // Small engines
        }
        ...
        if (mileage && mileage < 50000)
            factors.positive.push('Low mileage');
        if (condition === 'excellent')
            factors.positive.push('Excellent condition');
        if (age < 3)
            factors.positive.push('Recent model year');
        if (fuelType === 'diesel')
            factors.positive.push('Fuel efficient (diesel)');
        if (mileage && mileage > 150000)
            factors.negative.push('High mileage');
        if (age > 10)
            factors.negative.push('Older vehicle');
        if (condition === 'poor')
            factors.negative.push('Poor condition');
        const valuation = {
            vin: vin || null,
        ...
        firebase_functions_1.logger.info(`Valuation completed for ${make} ${model} ${year}`, {
            userId,
            vin,
        ...
        firebase_functions_1.logger.error('Error in GLOUBUL Valuation API:', error);
 ...
 */
exports.b2bMarketInsightsAPI = (0, https_1.onCall)({
    cors: true,
    region: 'europe-west1'
}, async (request) => {
    if (!request.auth) {
    ...
    const { region, // 'sofia', 'plovdiv', 'varna', 'burgas', 'all'
    ...
    try {
        // Track API usage
        await (0, subscriptions_1.trackAPIUsage)(userId, 'market-insights');
    ...
    `;
        ...
        firebase_functions_1.logger.info(`Market insights generated for ${region || 'all regions'}`, {
            userId,
            totalListings: insights.totalListings,
            tier: subscription.tier
        });
        ...
        firebase_functions_1.logger.error('Error in GLOUBUL Market Insights API:', error);
...
function generateMarketRecommendations(insights, health, category) {
...
//# sourceMappingURL=business-apis.js.map