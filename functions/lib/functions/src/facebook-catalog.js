...
exports.facebookCatalogCSV = exports.facebookCatalogXML = void 0;
 ...
 */
exports.facebookCatalogXML = functions.https.onRequest(async (req, res) => {
    try {
        // Set CORS headers
        ...
        cars.forEach((car) => {
            ...
            const image = ((_a = car.images) === null || _a === void 0 ? void 0 : _a[0]) || '';
            xml += '    <item>\n';
            ...
            if (car.images && car.images.length > 1) {
                car.images.slice(1, 10).forEach((img) => {
            ...
            // Vehicle-specific
            if (car.year)
                xml += `      <g:year>${car.year}</g:year>\n`;
            if (car.make)
                xml += `      <g:make>${escapeXML(car.make)}</g:make>\n`;
            if (car.model)
                xml += `      <g:model>${escapeXML(car.model)}</g:model>\n`;
            if (car.mileage)
                xml += `      <g:mileage>${car.mileage} km</g:mileage>\n`;
            if (car.vehicleType)
                xml += `      <g:vehicle_type>${escapeXML(car.vehicleType)}</g:vehicle_type>\n`;
            if (car.fuelType)
                xml += `      <g:fuel_type>${escapeXML(car.fuelType)}</g:fuel_type>\n`;
            if (car.transmission)
                xml += `      <g:transmission>${escapeXML(car.transmission)}</g:transmission>\n`;
            if (car.color)
                xml += `      <g:exterior_color>${escapeXML(car.color)}</g:exterior_color>\n`;
            // Location
            if (car.city)
                xml += `      <g:location>${escapeXML(car.city)}, ${escapeXML(car.region || 'Bulgaria')}</g:location>\n`;
            xml += '    </item>\n';
        });
        xml += '  </channel>\n';
        xml += '</rss>';
 ...
 */
exports.facebookCatalogCSV = functions.https.onRequest(async (req, res) => {
    try {
        // Set CORS headers
        res.set('Access-Control-Allow-Origin', '*');
...
//# sourceMappingURL=facebook-catalog.js.map