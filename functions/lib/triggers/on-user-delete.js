    ...
    try {
        ...
        if (userNumericId) {
            try {
                ...
                if (channelsSnapshot.exists()) {
                    ...
                    for (const [channelId, channelData] of Object.entries(channels)) {
                        ...
                        if (channel.buyerFirebaseId === userId || channel.sellerFirebaseId === userId) {
                            ...
                            deletePromises.push(rtdb.ref(`channels/${channelId}`).remove()
                                .then(() => {
                ...
                try {
                    ...
                    if (typingSnapshot.exists()) {
                        ...
                        for (const [channelId, channelTyping] of Object.entries(typing)) {
                            const channel = channelTyping;
                            if (channel[String(userNumericId)]) {
                                typingDeletePromises.push(rtdb.ref(`typing/${channelId}/${userNumericId}`).remove());
                            }
                        }
                ...
                errors.push(`realtime_database: ${error.message}`);
        ...
        try {
            ...
            errors.push(`posts_subcollections: ${error.message}`);
        }
        ...
        try {
            ...
            errors.push(`team_memberships: ${error.message}`);
        }
        ...
        try {
            ...
            errors.push(`storage_profile_pictures: ${error.message}`);
            // Not critical, continue
        }
...
//# sourceMappingURL=on-user-delete.js.map