...
exports.dailyAutoRenewal = void 0;
...
exports.dailyAutoRenewal = functions
    .region('europe-west1')
    .pubsub.schedule('0 2 * * *')
...
async function renewListing(car) {
    try {
...
async function sendRenewalNotification(userId, carId) {
    try {
        const userSnapshot = await db.collection('users').where('uid', '==', userId).get();
        if (userSnapshot.empty) {
...
async function saveRenewalReport(results) {
    try {
        const report = {
            totalProcessed: results.length,
            totalRenewed: results.filter(r => r.success).length,
...
//# sourceMappingURL=auto-renewal-cron.js.map