 ...
 */
function mapICardStatus(icardStatus) {
    switch (icardStatus) {
        case 'SUCCESS':
            return 'succeeded';
        case 'FAILED':
 ...
 */
async function processICardPayment(payload) {
    const { transactionId, amount, currency, status, metadata } = payload;
    if (!(metadata === null || metadata === void 0 ? void 0 : metadata.ad_id) || !(metadata === null || metadata === void 0 ? void 0 : metadata.user_id)) {
    ...
    const idempotencyKey = (0, webhook_verification_1.generateIdempotencyKey)('icard', transactionId, status);
    ...
    await db.runTransaction(async (transaction) => {
        ...
        const paymentRecord = Object.assign({ id: paymentRef.id, provider: 'icard', provider_tx_id: transactionId, ad_id: adId, user_id: userId, amount,
            currency, status: paymentStatus, idempotency_key: idempotencyKey, metadata, raw_webhook: payload, created_at: admin.firestore.Timestamp.now(), updated_at: admin.firestore.Timestamp.now() }, (paymentStatus === 'succeeded' && { settled_at: admin.firestore.Timestamp.now() }));
 ...
 */
exports.icardWebhooks = functions
    .region('europe-west1')
    ...
    const signature = req.headers['x-icard-signature'];
    if (!signature) {
    ...
    const verification = (0, webhook_verification_1.verifyICardSignature)(req.rawBody, signature, webhookSecret);
...
//# sourceMappingURL=icard-webhooks.js.map