...
class IndexNowService {
    ...
    static async submitUrls(urls) {
        ...
        const request = {
            ...
            urlList: urls.map(url => url.startsWith('http') ? url : `https://${SITE_HOST}${url}`),
        };
        const results = [];
...
// ============================================================================
exports.submitToIndexNow = functions.https.onCall(async (data, context) => {
    // Check authentication
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated to submit URLs');
    }
    const { urls } = data;
    if (!urls || !Array.isArray(urls)) {
        throw new functions.https.HttpsError('invalid-argument', 'URLs must be an array');
    }
    try {
...
exports.default = IndexNowService;
