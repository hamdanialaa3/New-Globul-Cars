# Fix Remaining @/ Imports - FAST VERSION
$ErrorActionPreference = "Continue"
$projectRoot = Get-Location
$packagesRoot = Join-Path $projectRoot "packages"

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Fix Remaining @/ Imports (FAST)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# Function to update imports in a file (FAST - reads only if contains @/)
function Update-FileImports {
    ...
    try {
        # Quick check: does file contain @/ imports?
        $quickCheck = Select-String -Path $FilePath -Pattern "from ['\`"]@/" -Quiet -ErrorAction SilentlyContinue
        if (-not $quickCheck) {
            return $false
        }
...
foreach ($package in $packages) {
    ...
    # Get files that contain @/ imports first (FAST)
    $filesWithImports = Get-ChildItem -Path $packagePath -Recurse -Include "*.ts", "*.tsx" -ErrorAction SilentlyContinue | Where-Object {
        ...
        (Select-String -Path $_.FullName -Pattern "from ['\`"]@/" -Quiet -ErrorAction SilentlyContinue)
    }
    
    $packageUpdated = 0
    $packageTotal = $filesWithImports.Count
    $counter = 0
    
    foreach ($file in $filesWithImports) {
        $totalFiles++
        $counter++
        if ($counter % 10 -eq 0) {
            Write-Host "  Progress: $counter/$packageTotal" -ForegroundColor Gray
        }
