...
export const aiGenerateCarDescription = functions.https.onCall(async (data, context) => {
  ...
  `;
    ...
    const proxy = new DeepSeekProxy();
    // ... (Duplicate quota check logic or refactor. For simplicity, we repeat essential checks or rely on the proxy if we had a service layer. 
    // Ideally, aiGenerateText logic should be in a service function, not just the http handler.
    // For now, let's just call the proxy logic directly.)
