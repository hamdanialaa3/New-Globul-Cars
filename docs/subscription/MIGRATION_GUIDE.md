...
**Status:** Phase 2 Complete — 4 remaining consumers  
**Priority:** High
...
| billing-service.ts      | ⚠️ @deprecated | `@deprecated` JSDoc added; 4 consumers remain |
| billing-operations.ts   | ⚠️ To Review   | Check if still needed                         |
| billing-data.ts         | ⚠️ To Remove   | LEGACY - marked deprecated                    |

### Remaining consumers of `billing-service.ts`

| File                                               | Symbol imported                     | Migration path                                            |
| -------------------------------------------------- | ----------------------------------- | --------------------------------------------------------- |
| `src/providers/StripeProvider.tsx`                 | `getStripeInstance`                 | Use `loadStripe(VITE_STRIPE_PK)` directly                 |
| `src/pages/08_payment-billing/InvoicesPage.tsx`    | invoice helpers                     | Move helpers to `src/services/billing/invoice-helpers.ts` |
| `src/pages/03_user-pages/billing/CheckoutPage.tsx` | `stripeClientService`               | Switch to `subscriptionService`                           |
| `src/services/platform-operations.ts`              | `stripeService.createPaymentIntent` | Use Firebase callable function `createCarPaymentIntent`   |
...
**Last Updated**: June 2026 — `@deprecated` added to billing-service.ts; 4 remaining consumers documented above
