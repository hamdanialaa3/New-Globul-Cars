...
import type { BulgarianUser, PrivateProfile } from '@globul-cars/core/types/user/bulgarian-user.types';

// Type alias for backward compatibility
type BulgarianUserProfile = BulgarianUser;
...
export class SocialAuthService {
   ...
   */
  static async createOrUpdateBulgarianProfile(user: User, additionalData?: Partial<BulgarianUser>): Promise<BulgarianUser> {
    ...
    if (userDoc.exists()) {
      ...
      const existingProfile = userDoc.data() as BulgarianUser;
      const updatedProfile: Partial<BulgarianUser> = {
      ...
      return { ...existingProfile, ...updatedProfile } as BulgarianUser;
    } else {
      // Create new profile - use PrivateProfile as default
      const newProfile: PrivateProfile = {
        uid: user.uid,
        email: user.email || '',
        displayName: user.displayName || '',
   ...
   */
  static async getBulgarianUserProfile(uid: string): Promise<BulgarianUser | null> {
    try {
   ...
   */
  static async updateBulgarianProfile(uid: string, updates: Partial<BulgarianUser>): Promise<void> {
    try {
      const userRef = doc(db, 'users', uid);
      await updateDoc(userRef, {
        ...updates,
  ...
  static async searchUsers(criteria: {
    ...
    try {
      ...
      if (criteria.profileType) {
        q = query(q, where('profileType', '==', criteria.profileType));
      }
      ...
      const users: BulgarianUser[] = [];
      
      querySnapshot.forEach((doc) => {
        users.push(doc.data() as BulgarianUser);
      });
