...
export const AIAnalyzingStep: React.FC<AIAnalyzingStepProps> = ({
  ...
  const currentLanguage = language; // Alias for backward compatibility
          ...
          >
            ...
            <div>
              <ProgressText style={{ marginBottom: '0.5rem' }}>
                {t.errorTitle[currentLanguage as keyof typeof t.errorTitle]}
              </ProgressText>
            ...
            <div style={{ display: 'flex', gap: '1rem', width: '100%', maxWidth: '400px' }}>
              <GlassButton
                ...
                fullWidth
                onClick={handleRetry}
                title={t.retry[currentLanguage as keyof typeof t.retry]}
                aria-label={t.retry[currentLanguage as keyof typeof t.retry]}
              >
                {t.retry[currentLanguage as keyof typeof t.retry]}
              </GlassButton>
              <GlassButton
                variant="primary"
                fullWidth
                onClick={() => onError(error)}
                title={t.manualEntry[currentLanguage as keyof typeof t.manualEntry]}
                aria-label={t.manualEntry[currentLanguage as keyof typeof t.manualEntry]}
              >
                {t.manualEntry[currentLanguage as keyof typeof t.manualEntry]}
              </GlassButton>
            </div>
          </ErrorContainer>
        </GlassCard>
