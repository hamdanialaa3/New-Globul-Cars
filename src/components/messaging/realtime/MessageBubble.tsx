...
import { CarEmojiRenderer } from '../CarEmojiRenderer';
...
const Bubble = styled.div<{ $isOwn: boolean; $type: string }>`
  ...
  border: 1px solid ${({ $isOwn }) =>
    $isOwn ? 'rgba(59, 130, 246, 0.3)' : 'var(--border-color, rgba(0, 0, 0, 0.1))'};
...
const StatusIcon = styled.span<{ $isRead: boolean }>`
  color: ${({ $isRead }) => ($isRead ? '#3B82F6' : 'var(--text-tertiary, rgba(0, 0, 0, 0.3))')};
  display: flex;
  align-items: center;
  transition: color 0.3s ease;
      ...
      case 'image': {
        ...
        return (
          <ImageContainer>
            ...
            {message.content && (
              <MessageText>
                <CarEmojiRenderer text={message.content} />
              </MessageText>
            )}
          </ImageContainer>
        );
