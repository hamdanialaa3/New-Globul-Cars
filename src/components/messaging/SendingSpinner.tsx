...
const IconWrapper = styled.div<{ status: 'sending' | 'sent' | 'delivered' | 'read' | 'failed' }>`
  ...
  align-items: center;
  color: ${props => {
    if (props.status === 'failed') return props.theme.colors.error.main || '#ef4444';
    if (props.status === 'read') return props.theme.colors.primary.main || '#3b82f6';
    if (props.status === 'delivered') return props.theme.colors.success.main || '#10b981';
