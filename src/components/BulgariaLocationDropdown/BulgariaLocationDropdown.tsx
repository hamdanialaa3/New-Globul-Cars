...
export const BulgariaLocationDropdown: React.FC<BulgariaLocationDropdownProps> = ({
  ...
  useEffect(() => {
    if (value.locationData?.city && cities.length > 0 && !showManualCity && !value.postalCode) {
      const selectedCity = cities.find(c => c.name === value.locationData?.city || c.nameEn === value.locationData?.city);
  ...
  }, [value.locationData?.city, cities, showManualCity]);
