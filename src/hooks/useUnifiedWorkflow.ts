...
export const useUnifiedWorkflow = (currentStep: number) => {
  ...
  useEffect(() => {
    const unsubscribeTimer = unifiedWorkflowPersistence.subscribeToTimer(
      (state) => {
        setTimerState(state);
      }
    );
    ...
    const unsubscribeClear = unifiedWorkflowPersistence.subscribeToClear(() => {
  ...
  const markStepCompleted = useCallback(() => {
    unifiedWorkflowPersistence.markStepCompleted(currentStep);
    setWorkflowData((prev) => {
      if (!prev) return prev;
  ...
  const isStepCompleted = useCallback(() => {
    return unifiedWorkflowPersistence.isStepCompleted(currentStep);
  }, [currentStep]);
  ...
  const getProgress = useCallback(() => {
    return unifiedWorkflowPersistence.getProgress();
  }, []);
  ...
  const validate = useCallback((strict: boolean = false): ValidationResult => {
    return unifiedWorkflowPersistence.validateData(strict);
  }, []);
  ...
  const markAsPublished = useCallback(() => {
    unifiedWorkflowPersistence.markAsPublished();
    setTimerState({
      isActive: false,
      remainingSeconds: 0,
      totalSeconds: 1200
