/**
 * AI Dashboard Page
 * لوحة تحكم الذكاء الاصطناعي للمشرفين
 * 
 * @page AIDashboard
 * @description لوحة تحكم شاملة لإدارة نظام الذكاء الاصطناعي
 */
      ...
      const mockAlerts: SecurityAlert[] = [
        {
          id: '1',
          severity: 'high',
          message: 'معدل طلبات غير عادي من المستخدم #1234',
          timestamp: new Date()
        },
        {
          id: '2',
          severity: 'medium',
          message: 'محاولات فاشلة متكررة من IP 192.168.1.1',
          timestamp: new Date(Date.now() - 3600000)
        }
      ];
      setSecurityAlerts(mockAlerts);

      // بيانات الإيرادات (محاكاة)
      setRevenueData([
        { month: 'يناير', revenue: 1200 },
        { month: 'فبراير', revenue: 1450 },
        { month: 'مارس', revenue: 1800 },
        { month: 'أبريل', revenue: 2100 },
        { month: 'مايو', revenue: 2400 },
        { month: 'يونيو', revenue: 2800 }
      ]);

      // بيانات الاستخدام (محاكاة)
      setUsageData([
        { name: 'وصف السيارات', value: 450 },
        { name: 'تحليل الأسعار', value: 320 },
        { name: 'ردود ذكية', value: 280 },
        { name: 'تحليل الصور', value: 150 }
      ]);
  ...
  return (
    <Container>
      <Header>
        ...
        <Subtitle>مراقبة شاملة لأداء وأمان نظام AI</Subtitle>
      </Header>
      ...
      <MetricsGrid>
        <MetricCard color="#3498db">
          <MetricHeader>
            <MetricTitle>إجمالي الطلبات</MetricTitle>
          ...
          <MetricTrend positive={metrics.dailyGrowth > 0}>
            ↑ {metrics.dailyGrowth}% نمو يومي
          </MetricTrend>
        </MetricCard>

        <MetricCard color="#27ae60">
          <MetricHeader>
            <MetricTitle>التكلفة الشهرية</MetricTitle>
          ...
          <MetricTrend positive={metrics.monthlyCost < metrics.monthlyBudget}>
            {((metrics.monthlyCost / metrics.monthlyBudget) * 100).toFixed(1)}% من الميزانية
          </MetricTrend>
        </MetricCard>

        <MetricCard color="#9b59b6">
          <MetricHeader>
            <MetricTitle>المستخدمون النشطون</MetricTitle>
          ...
          <MetricTrend positive={metrics.suspendedUsers === 0}>
            {metrics.suspendedUsers} معلق
          </MetricTrend>
        </MetricCard>

        <MetricCard color="#e67e22">
          <MetricHeader>
            <MetricTitle>متوسط التقييم</MetricTitle>
          ...
          <MetricTrend positive={metrics.averageRating >= 4}>
            {metrics.averageRating >= 4 ? 'ممتاز' : 'جيد'}
          </MetricTrend>
        </MetricCard>
      </MetricsGrid>
      ...
      <ChartsSection>
        ...
        <ChartCard>
          <ChartTitle>📈 الاستخدام حسب الميزة</ChartTitle>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                data={usageData}
      ...
      {/* التنبيهات الأمنية */}
      <AlertsSection>
        <ChartTitle>🛡️ التنبيهات الأمنية</ChartTitle>
        <AlertsList>
          {securityAlerts.length === 0 ? (
            <AlertItem severity="low">
              <AlertIcon>✅</AlertIcon>
              <AlertContent>
                <AlertMessage>لا توجد تنبيهات أمنية</AlertMessage>
                <AlertTime>النظام يعمل بشكل طبيعي</AlertTime>
              </AlertContent>
            </AlertItem>
          ) : (
      ...
      {/* الإجراءات السريعة */}
      <ChartTitle>⚡ إجراءات سريعة</ChartTitle>
      <ActionsSection>
