...
import { getVariantsForModel, modelHasVariants } from '../../../../services/carBrandsService';
 ...
 */
export const getRegistrationYear = (data) => {
    if (!data)
        return '';
    const rawYear = data.year || data.firstRegistration || '';
    if (!rawYear)
        return '';
...
export const useVehicleDataForm = () => {
    ...
    const initialValues = useMemo(() => {
        ...
        return Object.assign(Object.assign(Object.assign({}, defaultForm), (workflowData || {})), fromParams);
    ...
    const formEquals = useCallback((a, b) => {
    ...
    useEffect(() => {
        setFormData(prev => {
            ...
            const updated = formEquals(prev, initialValues) ? prev : Object.assign(Object.assign({}, initialValues), { 
                // Preserve bodyType if user has selected something
                bodyType: preservedBodyType, bodyTypeOther: preservedBodyTypeOther });
            return updated;
        });
        ...
        if (Object.keys(workflowData).length > 0) {
            ...
            if (hasSignificantData) {
                ...
                setFormData(prev => {
                    ...
                    restore('saleType', workflowData.saleType);
                    restore('saleTimeline', workflowData.saleTimeline);
                    restore('saleProvince', workflowData.saleProvince || workflowData.region);
                    restore('saleCity', workflowData.saleCity || ((_a = workflowData.locationData) === null || _a === void 0 ? void 0 : _a.cityName));
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            ...
            updateDataRef.current({
                ...
                roadworthy: (_a = formData.roadworthy) !== null && _a !== void 0 ? _a : undefined,
    ...
    useEffect(() => {
        ...
        brandsModelsDataService.getModelsForBrand(canonical)
            ...
            if (models.length === 0) {
                const candidate = structuredBrandsData[canonical];
                if (candidate === null || candidate === void 0 ? void 0 : candidate.models) {
                    const flattened = [];
                    Object.values(candidate.models).forEach((arr) => flattened.push(...arr));
    ...
    useEffect(() => {
        if (formData.make && formData.model) {
            ...
            if (hasVariants) {
                ...
                setFormData(prev => (prev.variant === '' ? prev : Object.assign(Object.assign({}, prev), { variant: '' })));
            }
        }
        else {
    ...
    const buildURLSearchParams = useCallback(() => {
        ...
        if (formData.make)
            params.set('mk', formData.make === '__other__' ? (formData.makeOther || '') : formData.make);
        if (formData.model)
            params.set('md', formData.model === '__other__' ? (formData.modelOther || '') : formData.model);
        if (formData.variant)
            params.set('variant', formData.variant === '__other__' ? (formData.variantOther || '') : formData.variant);
        if (formData.fuelType)
            params.set('fm', formData.fuelType === '__other__' ? (formData.fuelTypeOther || '') : formData.fuelType);
        if (formData.mileage)
            params.set('mi', formData.mileage);
        if (formData.transmission)
            params.set('tr', formData.transmission);
        if (formData.condition)
            params.set('condition', formData.condition); // ✅ ADDED: Include condition in URL
        ...
        if (fy)
            params.set('fy', fy);
        if (formData.color)
            params.set('color', (formData.color === 'other' || formData.color === '__other__') ? (formData.colorOther || '') : formData.color);
        return params;
