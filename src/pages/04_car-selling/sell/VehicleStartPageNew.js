...
const VehicleStartPageNew = () => {
    ...
    const handleSelect = async (typeId, disabled) => {
        var _a;
        // If disabled, show "Coming Soon" message
        if (disabled) {
            toast.info(language === 'bg' ? 'Скоро ще бъде налично' : 'Coming Soon', {
                autoClose: 3000,
                position: 'top-center'
            });
            return;
        ...
        const activeListings = ((_a = user === null || user === void 0 ? void 0 : user.stats) === null || _a === void 0 ? void 0 : _a.activeListings) || 0;
    ...
    const leftContent = (_jsx(ContentSection, { children: _jsx(VehicleGridCard, { children: _jsx(VehicleGrid, { children: vehicleTypes.map((vehicle) => {
                    const IconComponent = vehicle.IconComponent;
                    const isHovered = hoveredType === vehicle.id;
                    const isDisabled = vehicle.disabled || false;
                    return (_jsxs(VehicleOption, { "$isHovered": isHovered, "$disabled": isDisabled, onClick: () => handleSelect(vehicle.id, isDisabled), onMouseEnter: () => !isDisabled && setHoveredType(vehicle.id), onMouseLeave: () => setHoveredType(null), children: [_jsx(VehicleIconWrapper, { "$isHovered": isHovered, children: _jsx(IconComponent, {}) }), _jsx(VehicleLabel, { children: vehicle.id === 'parts'
                                    ? (language === 'bg' ? 'Резервни части' : 'Car Parts')
                                    : t(`sell.start.vehicleTypes.${vehicle.id}.title`) }), _jsx(VehicleDesc, { children: vehicle.id === 'parts'
                                    ? (language === 'bg' ? 'Резервни части и аксесоари' : 'Spare parts and accessories')
                                    : t(`sell.start.vehicleTypes.${vehicle.id}.desc`) }), isDisabled && (_jsx("div", { style: {
                                    marginTop: '0.5rem',
                                    fontSize: '0.875rem',
                                    fontWeight: 600,
                                    color: '#6c757d'
                                }, children: language === 'bg' ? 'Скоро' : 'Coming Soon' }))] }, vehicle.id));
                }) }) }) }));
    return (_jsx(SellWorkflowLayout, { currentStep: "vehicle-selection", children: _jsx(SplitScreenLayout, { leftContent: leftContent }) }));
};
