...
const DealerPublicPage: React.FC = () => {
  ...
  const pageImage = dealer.photoURL || '/logo.webp';
  ...
  return (
    <>
      ...
      <Container>
        {/* Hero Section */}
        <HeroSection>
          <ProfileImage src={dealer.photoURL || '/logo.webp'} alt={dealer.displayName} />
          <ProfileInfo>
            <NameContainer>
            ...
            <ListingsGrid>
              {listings.map((listing) => (
                <ListingCard key={listing.id} onClick={() => navigate(`/cars/${listing.id}`)}>
                  <ListingImage
                    src={listing.images?.[listing.featuredImageIndex || 0] || listing.images?.[0] || '/images/placeholder.png'}
