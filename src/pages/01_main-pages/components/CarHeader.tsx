...
// ✅ Styled Components for Trust Badges
const SellerBadgesContainer = styled.div`
  display: flex;
  gap: 0.5rem;
  align-items: center;
  margin-top: 0.5rem;
...
const SellerTypeBadge = styled.div<{ $type: 'dealer' | 'private' }>`
  display: inline-flex;
  align-items: center;
  gap: 0.4rem;
  font-size: 0.85rem;
...
export const CarHeader: React.FC<CarHeaderProps> = ({
  ...
  // ✅ Determine seller type (default to 'private' if not specified)
  const sellerType = (car.sellerType as 'dealer' | 'private' | undefined) || 'private';
  
  // ✅ Check if seller is verified (fallback to true for styling demo)
  const isVerified = (car as any).isVerified !== false;
  return (
    <Header>
      <TopBar>
        ...
        <SellerInfo>
          ...
          <SellerDetails>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              {/* ✅ Verification Badge */}
              {isVerified && (
                <TooltipContainer data-tooltip="Verified Identity">
                  <CheckCircle size={18} color="#3b82f6" fill="#3b82f6" style={{ marginBottom: '2px' }} />
                </TooltipContainer>
              )}
            </div>
            
            {/* ✅ Seller Type & Trust Badges */}
            <SellerBadgesContainer>
              <SellerTypeBadge $type={sellerType}>
                {sellerType === 'dealer' 
                  ? (language === 'bg' ? '🏢 Официален дилър' : '🏢 Official Dealer')
                  : (language === 'bg' ? 'Частен продавач' : 'Private Seller')
                }
              </SellerTypeBadge>
            </SellerBadgesContainer>
