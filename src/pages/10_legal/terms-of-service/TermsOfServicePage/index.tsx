...
const Container = styled.div`
  ...
  background: ${({ theme }) => {
    if (theme.mode === 'dark') {
      return theme.colors?.background?.paper || theme.colors.background.dark || '#1a1a2e';
    }
    return theme.colors?.background?.paper || theme.colors.background.light || '#ffffff';
  }};
  border-radius: 8px;
  box-shadow: 0 2px 8px ${({ theme }) => theme.mode === 'dark' ? 'rgba(0, 0, 0, 0.5)' : 'rgba(0, 0, 0, 0.1)'};
...
const ContactInfo = styled.div`
  background: ${({ theme }) => theme.mode === 'dark' ? 'rgba(30, 64, 175, 0.1)' : (theme.colors?.grey?.[100] || '#f8f9fa')};
  padding: 1.5rem;
  border-radius: 8px;
  margin: 1rem 0;
`;
...
const LastUpdated = styled.div`
  ...
  border-radius: 8px;
  font-style: italic;
  color: ${props => props.theme.colors?.text?.secondary || props.theme.text?.secondary || '#6c757d'};
`;
