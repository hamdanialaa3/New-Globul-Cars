...
// ❌ CRITICAL: Missing numeric ID - Assign one now!
  if (user.uid) {
    logger.warn('User missing numericId, assigning now...', {
      uid: user.uid,
      email: user.email
    });
    
    // ✅ CONSTITUTION-COMPLIANT: Assign numeric ID immediately
    // Import at top: import { ensureUserNumericId } from '../services/numeric-id-assignment.service';
    // Note: This is async but routing-utils functions are not async
    // Return a temporary URL that will redirect properly after numeric ID assignment
    logger.error('🚨 CRITICAL: User has no numericId! Redirecting to home to trigger assignment.');
    return '/'; // Redirect to home, ProfilePageWrapper will handle numeric ID assignment
  }
