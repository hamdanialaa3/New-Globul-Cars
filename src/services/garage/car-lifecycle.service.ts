...
import { doc, getDoc, updateDoc, serverTimestamp as firestoreTimestamp } from 'firebase/firestore';
...
export async function archiveCarConversations(
  ...
  if (!carNumericId || !carFirebaseId) {
    logger.warn('[CarLifecycle] Invalid parameters for archiveCarConversations', {
      carNumericId,
      carFirebaseId
    });
  ...
  try {
    ...
    if (!snapshot.exists()) {
      logger.info('[CarLifecycle] No channels found to archive', { carNumericId });
      return { archivedCount: 0 };
    }
    ...
    logger.error('[CarLifecycle] Failed to archive car conversations', error as Error, {
      carNumericId,
      carFirebaseId,
      newStatus
    });
...
export async function isChannelArchived(channelId: string): Promise<boolean> {
  try {
    ...
    logger.error('[CarLifecycle] Failed to check channel archive status', error as Error, {
      channelId
    });
    return false;
  }
...
export async function getChannelArchiveMetadata(channelId: string): Promise<{
  ...
  try {
    ...
    logger.error('[CarLifecycle] Failed to get archive metadata', error as Error, {
      channelId
    });
    return null;
  }
...
export async function reactivateChannel(channelId: string): Promise<void> {
  try {
    ...
    logger.error('[CarLifecycle] Failed to reactivate channel', error as Error, {
      channelId
    });
    throw error;
  }
