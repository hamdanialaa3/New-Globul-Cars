...
class UsageTrackingService {
  ...
  async getCurrentUsage(userId: string): Promise<UsageStats | null> {
    try {
      ...
      serviceLogger.error('Error getting current usage', error as Error, {
        userId,
      });
      return null;
    }
  ...
  shouldWarnUser(usage: UsageStats): boolean {
    return (
      (!usage.listings.unlimited &&
        usage.listings.percentage >= this.WARNING_THRESHOLD) ||
      (!usage.teamMembers.unlimited &&
        usage.teamMembers.percentage >= this.WARNING_THRESHOLD) ||
      (!usage.campaigns.unlimited &&
        usage.campaigns.percentage >= this.WARNING_THRESHOLD) ||
      (!usage.apiCalls.unlimited &&
        usage.apiCalls.percentage >= this.WARNING_THRESHOLD)
    );
  }
  ...
  shouldBlockUser(usage: UsageStats): boolean {
    return (
      ...
      (!usage.campaigns.unlimited &&
        usage.campaigns.current >= usage.campaigns.max) ||
      (!usage.apiCalls.unlimited &&
        usage.apiCalls.current >= usage.apiCalls.max)
    );
  }
  ...
  getUsageWarnings(usage: UsageStats): UsageWarning[] {
    ...
    if (
      ...
      warnings.push({
        ...
        severity:
          usage.listings.percentage >= this.CRITICAL_THRESHOLD
            ? 'error'
            : 'warning',
    ...
    if (
      ...
      warnings.push({
        ...
        severity:
          usage.teamMembers.percentage >= this.CRITICAL_THRESHOLD
            ? 'error'
            : 'warning',
    ...
    if (
      ...
      warnings.push({
        ...
        severity:
          usage.campaigns.percentage >= this.CRITICAL_THRESHOLD
            ? 'error'
            : 'warning',
    ...
    if (
      ...
      warnings.push({
        ...
        severity:
          usage.apiCalls.percentage >= this.CRITICAL_THRESHOLD
            ? 'error'
            : 'warning',
  ...
  async incrementUsage(userId: string, type: UsageType): Promise<boolean> {
    try {
      ...
      serviceLogger.error('Error incrementing usage', error as Error, {
        userId,
        type,
      });
      return false;
  ...
  async decrementUsage(userId: string, type: UsageType): Promise<boolean> {
    try {
      ...
      serviceLogger.error('Error decrementing usage', error as Error, {
        userId,
        type,
      });
      return false;
  ...
  async resetAPICallsCounter(userId: string): Promise<void> {
    try {
      ...
      serviceLogger.error('Error resetting API calls', error as Error, {
        userId,
      });
    }
  }
