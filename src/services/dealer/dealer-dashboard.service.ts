...
class DealerDashboardService {
  ...
  async getDashboardData(userId: string): Promise<DealerDashboardData> {
    try {
      const [stats, topListings, recentListings, alerts, tasks] = await Promise.all([
        this.getStats(userId),
        this.getTopListings(userId, 5),
        this.getRecentListings(userId, 10), // Fetch recent for Inventory Widget
      ...
      return {
        stats,
        topListings,
        recentListings, // Add to return object
        alerts,
        tasks
      };
    } catch (error) {
  ...
  /**
   * Get recent listings for Inventory Widget
   */
  async getRecentListings(userId: string, limitCount: number = 10): Promise<UnifiedCar[]> {
    try {
      const userCars = await unifiedCarService.getUserCars(userId);
      // Sort by updated/created recent first
      return userCars.sort((a, b) => {
        const dateA = a.updatedAt || a.createdAt;
        const dateB = b.updatedAt || b.createdAt;
        const timeA = (dateA as any).toMillis ? (dateA as any).toMillis() : new Date(dateA).getTime();
        const timeB = (dateB as any).toMillis ? (dateB as any).toMillis() : new Date(dateB).getTime();
        return timeB - timeA;
      }).slice(0, limitCount);
    } catch (error) {
      serviceLogger.error('Error getting recent listings', error as Error, { userId });
      return [];
    }
  }
      ...
      for (const car of activeCars) {
        ...
        if (images.length < 3) {
          alerts.push({
            ...
            createdAt: car.createdAt instanceof Timestamp ? car.createdAt : Timestamp.fromDate(new Date(car.createdAt as any))
          });
        }
