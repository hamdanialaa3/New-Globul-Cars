  ...
  static async createCarListing(
    ...
    try {
      ...
      try {
        ...
        // ✅ NEW: Cleanup Draft upon success (Phase 3)
        if (payload.draftId) {
          await SellWorkflowOperations.deleteDraft(userId, payload.draftId);
        }
      ...
      } catch (carCreationError) {
        // 🔥 ROLLBACK: Delete uploaded images if car creation failed
        logger.error('Car creation failed - rolling back uploaded images', carCreationError as Error);
        if (imageUrls.length > 0) {
          await Promise.all(imageUrls.map(url => SellWorkflowImages.deleteImage(url, userId).catch(e =>
            logger.warn('Failed to cleanup image during rollback', { url, error: e.message })
          )));
        }
        throw carCreationError;
      }
