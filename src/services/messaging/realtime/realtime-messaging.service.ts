...
export interface RealtimeChannel {
  ...
  buyerPhone?: string;          // Phone number for calling feature
  ...
  sellerPhone?: string;         // Phone number for calling feature
  carNumericId: number;
  carFirebaseId: string;
  carTitle: string;
  carPrice: number;
...
class RealtimeMessagingService {
  ...
  async getOrCreateChannel(params: CreateChannelParams): Promise<RealtimeChannel> {
    ...
    try {
      ...
      logger.warn('Failed to check block status before channel creation', {
        buyer: buyer.numericId,
        seller: seller.numericId,
        error: (error as Error).message
      });
  ...
  async sendMessage(
    ...
    try {
      ...
      logger.warn('Failed to check block status before sending message', {
        channelId,
        senderId: message.senderId,
        recipientId: message.recipientId,
        error: (error as Error).message,
  ...
  async markAsRead(
    channelId: string,
    userNumericId: string | number
  ): Promise<void> {
    try {
      const numericId = typeof userNumericId === 'string' ? parseInt(userNumericId) : userNumericId;

      if (isNaN(numericId)) {
        throw new Error('INVALID_NUMERIC_ID: userNumericId must be a valid number or numeric string');
      }

      // Reset unread count for this user in the channel
      const unreadRef = ref(this.db, `channels/${channelId}/unreadCount/${numericId}`);
      await set(unreadRef, 0);

      // Mark individual messages as read (optional optimization for display)
      const messagesRef = ref(this.db, `messages/${channelId}`);
      ...
      if (snapshot.exists()) {
        ...
        snapshot.forEach((childSnapshot) => {
          ...
          // Re-verify the check logic
          if (message.recipientId === numericId && !message.read) {
      ...
      logger.debug('[RealtimeMessaging] Marked messages as read', { channelId, numericId });
    } catch (error) {
      logger.error('[RealtimeMessaging] Failed to mark as read', error as Error, { channelId });
    }
  }
