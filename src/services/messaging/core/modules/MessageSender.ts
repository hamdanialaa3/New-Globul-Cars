...
export class MessageSender {
  ...
  async sendMessage(params: {
    ...
    try {
      ...
      const { sendMessage: sendRealtimeMessage } =
        await import('@/services/realtime-messaging-operations');
      const { advancedMessagingService } =
        await import('../../advanced-messaging-service');
      const { userService } =
        await import('../../../user/canonical-user.service');
      ...
      const senderName =
        senderProfile?.displayName || senderProfile?.firstName || 'User';
      const receiverName =
        receiverProfile?.displayName || receiverProfile?.firstName || 'User';
      ...
      try {
        ...
        logger.error(
          '[MessageSender] Firestore persistence failed',
          firestoreError as Error
        );
        throw new Error(
          `Failed to persist message: ${(firestoreError as Error).message}`
        );
      }

      // 5. Update delivery status via DeliveryEngine (after successful save)
      await deliveryEngine
        .updateStatus(conversationId, firestoreMessageId, 'sent')
        .catch(err => {
          logger.warn('[MessageSender] Delivery status update failed', {
            error: err.message,
          });
      ...
      this.processWithAI({ ...message, id: firestoreMessageId }).catch(err =>
        logger.warn('[MessageSender] AI processing failed', {
          error: err.message,
        })
      );
  ...
  private async processWithAI(message: any): Promise<void> {
    try {
      const { aiSmartSuggestionsService } =
        await import('../../ai-smart-suggestions.service');

      const suggestions = await aiSmartSuggestionsService.generateSuggestions({
        recentMessages: [
          {
            content: message.content,
            senderNumericId: Number(message.senderId) || 0,
            timestamp: Date.now(),
          },
        ],
        userRole: 'seller',
      ...
      if (suggestions.length > 0) {
        logger.info('[MessageSender] AI suggestions generated', {
