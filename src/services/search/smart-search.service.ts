...
import { CAR_AUTOCOMPLETE_DATA } from '../../data/car-autocomplete-data';
...
class SmartSearchService {
  ...
  async search(
    ...
    try {
      ...
      if (userId && keywords.trim()) {
        await searchHistoryService.saveSearch(userId, keywords, parsed as any);
      }
  ...
  async getSuggestions(
    ...
    try {
      ...
      const queryStr = partial.toLowerCase().trim();

      // 1. Get from static local data (FASTEST)
      // PRIORITY 1: Starts With (Exact match at beginning)
      CAR_AUTOCOMPLETE_DATA.categories.forEach(cat => {
        if (cat.toLowerCase().startsWith(queryStr)) suggestions.add(cat);
      });

      CAR_AUTOCOMPLETE_DATA.brands.forEach(brand => {
        if (brand.toLowerCase().startsWith(queryStr)) suggestions.add(brand);
      });

      if (suggestions.size < limit) {
        CAR_AUTOCOMPLETE_DATA.models.forEach(model => {
          if (model.toLowerCase().startsWith(queryStr)) {
            suggestions.add(model);
          }
        });
      }

      // PRIORITY 2: Includes (Only if space left and query is at least 2 chars)
      if (suggestions.size < limit && queryStr.length >= 2) {
        CAR_AUTOCOMPLETE_DATA.brands.forEach(brand => {
          if (suggestions.size < limit && brand.toLowerCase().includes(queryStr)) {
            suggestions.add(brand);
          }
        });
      }

      // 2. Get user's recent searches (if logged in)
      if (userId) {
        const recentSearches = await searchHistoryService.getRecentSearches(userId, 5);
        recentSearches
          .filter(s => s.query.toLowerCase().includes(queryStr))
      ...
      popularSearches
        .filter(s => s.toLowerCase().includes(queryStr))
        .forEach(s => suggestions.add(s));

      // 4. Get brand/model suggestions from Firestore (LATEST FALLBACK)
      const parsed = this.parseKeywords(partial);
      if (parsed.brands.length === 0 && partial.length >= 2 && suggestions.size < limit) {
  ...
  async getAutocompleteSuggestions(
  ...
  ): Promise<{ categories: any[]; makes: any[]; models: any[] }> {
    if (!query || query.trim().length < 1) {
      return { categories: [], makes: [], models: [] };
    }
    ...
    try {
      ...
      // 1. Match Categories
      CAR_AUTOCOMPLETE_DATA.categories.forEach(cat => {
        if (cat.toLowerCase().startsWith(q) && categories.length < 5) {
          categories.push({ value: cat, count: undefined });
        }
      });

      // 2. Match Brands (Makes)
      CAR_AUTOCOMPLETE_DATA.brands.forEach(brand => {
        if (brand.toLowerCase().startsWith(q) && makes.length < 5) {
          makes.push({ value: brand, count: undefined });
        }
      });

      // 3. Match Models
      CAR_AUTOCOMPLETE_DATA.models.forEach(model => {
        if (model.toLowerCase().startsWith(q) && models.length < maxSuggestions) {
          models.push({ value: model, count: undefined });
        }
      });
