...
export class SEOPrerenderService {
  ...
  generateCityPageSEO(cityData: CityPageData): SEOPrerenderData {
    const title = `Продажба на коли в ${cityData.cityBg} - Globul Cars`;
    const description = `Намерете идеалния автомобил в ${cityData.cityBg}. Над ${cityData.totalCars} обяви от частни лица, автосалони и компании. Средна цена: ${cityData.avgPrice} лв. Българска платформа за българските автомобилисти.`;
