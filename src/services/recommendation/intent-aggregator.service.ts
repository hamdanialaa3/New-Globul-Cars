...
class IntentAggregatorService {
  ...
  async getUnifiedIntent(forceRefresh = false): Promise<UnifiedIntent> {
    ...
    try {
      ...
      const [externalPrefs, crossPlatformPrefs, internalBehavior] =
        await Promise.all([
      ...
      const bodyTypes = this.aggregateBodyTypes(
        externalPrefs,
        internalBehavior
      );

      // Aggregate price range
      const priceRange = this.aggregatePriceRange(
        externalPrefs,
        internalBehavior
      );

      // Aggregate year range
      const yearRange = this.aggregateYearRange(
        externalPrefs,
        internalBehavior
      );
      ...
      this.cachedIntent = {
        ...
        yearRange,
        preferredFuel:
          externalPrefs.preferredFuel || internalBehavior?.preferredFuel,
        preferredGearbox:
          externalPrefs.preferredGearbox || internalBehavior?.preferredGearbox,
        keywords,
        confidence,
   ...
   */
  private async getCrossPlatformPreferences(): Promise<
   ...
   */
  private async getInternalBehavior(): Promise<
    (UserBehavior & Record<string, any>) | null
  > {
    try {
      return behaviorService.getBehavior() as UserBehavior &
        Record<string, any>;
    } catch {
      return null;
    }
  ...
  private aggregateBrandPreferences(
    ...
    internal: (UserBehavior & Record<string, any>) | null
  ): BrandPreference[] {
    const brandScores: Record<
      string,
      {
        weight: number;
        sources: Set<string>;
    ...
    for (const item of external.brands) {
      ...
      const decayedWeight =
        item.frequency *
        SOURCE_WEIGHTS.externalSearch *
        calculateDecay(item.recency);
    ...
    if (internal) {
      ...
      for (const [brand, count] of (
        internal.viewedBrands || new Map<string, number>()
      ).entries()) {
      ...
      // Contacted brands
      for (const [brand, count] of (
        internal.contactedBrands || new Map<string, number>()
      ).entries()) {
        const normalized = normalizeBrand(brand);
        if (!brandScores[normalized]) {
          brandScores[normalized] = {
      ...
      // Favorite brands
      for (const [brand, count] of (
        internal.favoriteBrands || new Map<string, number>()
      ).entries()) {
        const normalized = normalizeBrand(brand);
        if (!brandScores[normalized]) {
          brandScores[normalized] = {
    ...
    const maxWeight = Math.max(
      ...Object.values(brandScores).map(b => b.weight),
      1
    );
  ...
  private aggregateBodyTypes(
    ...
    const bodyTypeScores: Record<
      string,
      { weight: number; sources: Set<string> }
    > = {};
    ...
    for (const item of external.bodyTypes) {
      ...
      bodyTypeScores[type].weight +=
        item.frequency * SOURCE_WEIGHTS.externalSearch;
    ...
    if (internal?.viewedBodyTypes) {
      for (const [type, count] of (
        internal.viewedBodyTypes as Map<string, number>
      ).entries()) {
  ...
  private aggregatePriceRange(
    ...
    // Average all sources
    const mins = prices
      .filter(p => p.min !== undefined)
      .map(p => p.min as number);
    const maxs = prices
      .filter(p => p.max !== undefined)
      .map(p => p.max as number);
  ...
  private aggregateYearRange(
    ...
    const mins = years
      .filter(y => y.min !== undefined)
      .map(y => y.min as number);
    const maxs = years
      .filter(y => y.max !== undefined)
      .map(y => y.max as number);
  ...
  private aggregateKeywords(
    ...
    if (internal?.searches) {
      for (const search of internal.searches) {
        const term = [search.brand, search.model, search.fuel, search.bodyType]
          .filter(Boolean)
          .join(' ');
        const words = term
          .toLowerCase()
