  ...
  static async saveWorkflowData(
    ...
    try {
      ...
      if (!rateLimitResult.allowed) {
        logger.warn('Workflow save rate limited', { userId, retryAfter: rateLimitResult.retryAfter });
        return {
          success: false,
  ...
  static async updateWorkflowData(
    ...
    try {
      ...
      if (!rateLimitResult.allowed) {
        return {
          success: false,
          error: `Too many updates. Try again in ${rateLimitResult.retryAfter} seconds.`
        };
      }
