...
export class CanonicalUserService {
  ...
  private validateUserData(userData: any): void {
    ...
    // ✅ FIX: Email is optional for all users (guests, OAuth users without email permission, etc.)
    // Only log warning if regular user has no email
    if (!userData.email && !userData.isAnonymous) {
      logger.warn('User profile loaded without email', { 
        uid: userData.uid,
