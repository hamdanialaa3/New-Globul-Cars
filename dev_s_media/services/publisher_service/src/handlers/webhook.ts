...
import { socialQueue } from '../redis/queue';
...
export const handleAdPublished = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const existing = await client.query(
            'SELECT post_id, status FROM social_posts WHERE idempotency_key = $1',
            [idempotencyKey]
        );
        ...
        const insertRes = await client.query(
            `INSERT INTO social_posts (ad_id, platform, idempotency_key, status) 
       VALUES ($1, 'all', $2, 'queued') 
       RETURNING post_id`,
            [ad_id, idempotencyKey]
        );
        const postId = insertRes.rows[0].post_id;

        // 5. Add to Redis Queue
        await socialQueue.add('publish-job', {
            adId: ad_id,
            postId: postId,
            content: content,
            traceId: requestId
